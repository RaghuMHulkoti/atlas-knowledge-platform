...
| **Embeddings**   | **Google Generative AI** (Gemini)  | Turns text into vectors via `gemini-embedding-001` (3072-dim). Requires `GOOGLE_API_KEY`.                        |
...
[open & edit](https://excalidraw.com/#json=Uy6T8pFoK1M1H868teYyV,T3Gwx1fljymxyyKdwyeLdA)
...
4. **Embed** — `EmbeddingService` + the Google Gemini provider
   (`gemini-embedding-001`) turn each chunk into a 3072-dim vector.
...
1. **Embed the query** — `ChromaRetriever` embeds the query with the *same*
   Gemini model used at index time, so the vectors are comparable.
